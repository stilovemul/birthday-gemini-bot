...
async def api_dispatch_command(req: DispatchCommandRequest):
    ...
    try:
        ...
        elif "антиспам" in lower or "antispam" in lower:
            text = (
                "📵 <b>Антиспам-фильтр & Защита от спама</b>\n\n"
                "Пришлите подозрительный номер телефона или текст сообщения — я проверю его по базам спамеров, коллекторов и мошенников."
            )
        elif "авто-щит" in lower or "автощит" in lower:
            text = (
                "🛡 <b>Авто-Щит: Проверка авто перед покупкой</b>\n\n"
                "Я помогаю проверить автомобиль перед покупкой: пришлите ссылку на объявление (Авито, Авто.ру) или VIN/госномер — разберу скрытые дефекты, историю ДТП и признаки перекупов!"
            )
        elif "сейф" in lower or "vault" in lower:
            text = (
                "🔐 <b>Секретный сейф приватных данных</b>\n\n"
                "Все ваши секретные записи хранятся в зашифрованном виде.\n"
                "Чтобы добавить запись в сейф, напишите:\n<code>/vault Текст записи</code>"
            )
        elif "заметк" in lower or "notes" in lower:
            text = (
                "📝 <b>Быстрые заметки</b>\n\n"
                "Чтобы сохранить новую мысль или задачу, напишите:\n<code>/note Текст заметки</code>"
            )
