...
async def find_country_resorts(
    ...
    if is_another or seen:
        variation_instruction = (
            ...
            f"{seen_text}\n"
            f"ОБЯЗАТЕЛЬНО предложи совершенно ДРУГУЮ загородную базу/отель/курорт/глэмпинг/баню!"
        )

    prompt = f"""Ты — персональный эксперт-консьерж по загородному семейному отдыху в Санкт-Петербурге, Ленинградской области и Карелии.
Категория: {category}
Запрос пользователя / Пожелания / Бюджет: '{query}'
{variation_instruction}

Подбери ОДНУ конкретную идеальную загородную базу отдыха, семейный курорт, коттеджный клуб, спа-отель, глэмпинг или банный комплекс под этот запрос.
Укажи честные актуальные данные: реальное расстояние от СПб, трассу, цены, особенности инфраструктуры и лайфхак бронирования.
Фокусируйся на загородном семейном отдыхе, комфорте с детьми, природе, развлечениях и инфраструктуре.

КАТЕГОРИЧЕСКИЙ ЗАПРЕТ: НЕ ПРЕДЛАГАЙ отдельные рестораны или кафе! Это модуль загородного проживания, курортов, баз отдыха и коттеджей!

СТРУКТУРА JSON:
{{
  "name": "Название базы / курорта / отеля / глэмпинга",
  "category": "Короткий атмосферный статус / категория (например: 👨‍👩‍👧 Семейный загородный курорт или 🏊‍♂️ Спа & Теплый открытый бассейн)",
  ...
  "features": "Бассейн с подогревом, русская баня, спа, купель, ресторан на территории, прокат лодок, мангалы",
  "kid_friendly": "Что есть для детей и семьи (площадки, анимация, детские комнаты, кролики, мини-ферма, веревочный парк, pet-friendly)",
  "why_best": "Почему это место идеально попадает в запрос для загородного семейного отдыха (1-2 предложения)",
